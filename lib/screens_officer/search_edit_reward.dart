...
class _SearchAndEditRewardPageState extends State<SearchAndEditRewardPage> {
  ...
  Future<List<dynamic>> _fetchRewards([String query = '']) async {
    final response = await http.get(Uri.parse('http://192.168.0.122:3000/rewards?query=$query'));
  ...
  Future<void> _updateReward(String rewardId, String rewardName, int pointsRequired, String description, int quantity, File? imageFile) async {
    var uri = Uri.parse('http://192.168.0.122:3000/rewards/$rewardId');
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: ClipOval(
                              child: Image.network(
                                'http://192.168.0.122:3000/uploads/${reward['image']}',
                                width: 50,
                                height: 50,
                                fit: BoxFit.cover,
